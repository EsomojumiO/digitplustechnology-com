...
match the anonymised hospital case study — so the row above, not the code, is
what the follow-up is tracked by, and it is deliberately kept actionable.
Restore the name and employer in code only alongside written approval. The
other two are still named in code.
