...
|---|---|---|---|---|
| H1 | **Real testimonials** — *this row described role-based placeholders; superseded by `47ecd3c` (2026-07-17).* **Actual state:** five real Google reviews ship (`approved: true`, quotable by publication). Three further testimonials are **drafts we wrote, attributed to named real people** — not words those people said. They sit `approved: false`, render nowhere, and **must not** be published without written sign-off from the named person. Send status of the drafts is **unconfirmed** — the repo has no evidence they were ever sent. Tracking record: `PLACEHOLDERS.md` → "PENDING — 3 direct testimonials" | Client | Written sign-off from each named person, then set `approved: true` **with** `approvedBy` + `approvedDate` — the type requires both and the build fails otherwise. Publish what they send back, not the draft. Do **not** flip the flag to make the section look fuller | `18-launch-readiness.md` §15 · `PLACEHOLDERS.md` · C13, BLOCKERS #3 |
