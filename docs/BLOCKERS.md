...
## Content — missing cover image

| # | Item | Current state | Where to enable |
|---|------|---------------|-----------------|
| 19 | **Cover image for `it-asset-procurement-hospitals-nigeria`** (hospital case study, published 2026-08-15) | Frontmatter points at `/images/insights/it-asset-procurement-hospitals-nigeria.jpg`, which does not exist yet (loader tolerates it; the image 404s). | Set `UNSPLASH_ACCESS_KEY` and run `node content-engine/fetch-covers.mjs --slug it-asset-procurement-hospitals-nigeria`, or drop a 1600×900 JPG at that path (a real photo from the deployment would be strongest). |
| 20 | **Cover image for `school-computer-lab-ipele-ondo-starlink`** (Ipele school lab case study, published 2026-08-15) | Frontmatter points at `/images/insights/school-computer-lab-ipele-ondo-starlink.jpg`, which does not exist yet (loader tolerates it; the image 404s). | Set `UNSPLASH_ACCESS_KEY` and run `node content-engine/fetch-covers.mjs --slug school-computer-lab-ipele-ondo-starlink`, or drop a 1600×900 JPG at that path (a real photo from the Ipele build would be strongest). |
| 21 | **Cover image for `microsoft-entra-intune-small-business-nigeria`** (law-practice Entra/Intune case study, published 2026-08-15) | Frontmatter points at `/images/insights/microsoft-entra-intune-small-business-nigeria.jpg`, which does not exist yet (loader tolerates it; the image 404s). | Set `UNSPLASH_ACCESS_KEY` and run `node content-engine/fetch-covers.mjs --slug microsoft-entra-intune-small-business-nigeria`, or drop a 1600×900 JPG at that path. |
